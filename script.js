...
And I wanna stay with you until we're grey and old.`;  } else {
...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "Con amor, Castro";
...
function showCountdown() { {
  ...
  // Fechas base
  let startDate = startParam ? new Date(startParam + 'T00:00:00') : new Date('2024-07-31T00:00:00'); 
  let eventDate = eventParam ? new Date(eventParam + 'T00:00:00') : new Date('2026-07-31T00:00:00');

  function update() {
    const now = new Date();
    
    // CÁLCULO PARA "LLEVAMOS JUNTOS"
    ...
    container.innerHTML =
      `Juntos desde : <b>${days}d ${hours}h ${minutes}m ${seconds}s</b><br>` +
  ...
  update();
  setInterval(update, 1000); // Esto hace que ambos contadores se muevan cada segundo
}
...
function playBackgroundMusic() {
  ...
  // Intentar cargar la música personalizada si existe el parámetro
  ...
  // FUNCIÓN MÁGICA: Reproduce al primer toque en CUALQUIER parte
  const startAudio = () => {
    audio.play().then(() => {
      // Una vez que empieza a sonar, quitamos los escuchas para no saturar
  ...
  // Escuchar clic (PC) o toque (Móvil) en todo el documento
  document.addEventListener('click', startAudio);
  document.addEventListener('touchstart', startAudio);

  // Mantener tu botón por si ella quiere pausarla después
  let btn = document.getElementById('music-btn');
  if (!btn) {
    btn = document.createElement('button');
    btn.id = 'music-btn';
    btn.textContent = '▶️ Toca para Música'; // Texto inicial claro
    btn.style.position = 'fixed';
    btn.style.bottom = '18px';
    btn.style.right = '18px';
    btn.style.zIndex = 1000; // Asegurar que esté arriba de todo
